...
void loop() {
          dist = analogRead(sensorpin);              //lire les distance du capteur
 
          if(dist < object) {                        //si la distance est supérieur a 550 alors continuer en marche avant
          forward();                                
           }
          if(dist >= object) {              //si la distance est inferieur ou égal a 550
         findroute();
           }
}
 
void forward() {                            // marche avant
...
void findroute() {        // procédure de recherche d'une route
  halt();                                            // arret
...
void backward() {     // faire marche arrière
...
void lookleft() {                  //chercher un obstacle a gauche
...
void lookright () {     //chercher un obstacle a droite 
...
void turnleft () {            //tourner a gauche 
  digitalWrite(motor_pin1,HIGH);      //moteur de droite en marche avant
  digitalWrite(motor_pin2,LOW);      
  digitalWrite(motor_pin3,LOW);
...
void turnright () {         //tourner a droite
