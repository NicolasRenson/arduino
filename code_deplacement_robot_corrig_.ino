...
int servopin = 2;     // pin servomoteur capteur 
#define TRIGG 6 // Broche TRIGGER
#define ECHO 5    // Broche ECHO
int dist = 0;
int leftdist = 0;
int rightdist = 0;
...
void loop() 
{
 digitalWrite(TRIGG, HIGH); // Lance une mesure de distance en envoyant 
  ...
  int mesure = pulseIn(ECHO, HIGH, TIMEOUT); // Mesure le temps entre  l'envoi de l'ultrason et sa réception
  float dist = mesure / 2.0 * son; //calcul de la distance grâce au temps
...
{  //chercher un obstacle a gauche
    ...
    delay(700);                                //attendre que le servo prenne sa position 
      int mesure = pulseIn(ECHO, HIGH, TIMEOUT); // Mesure le temps entre  l'envoi de l'ultrason et sa réception
     float leftdist = mesure / 2.0 * son; ;           // lire la distance
...
{     
  ...
  delay(700);                 //attendre que le servo prenne sa position 
  int mesure = pulseIn(ECHO, HIGH, TIMEOUT); // Mesure le temps entre  l'envoi de l'ultrason et sa réception
 float rightdist = mesure / 2.0 * son;    
  servomoteur2.write(90);             // revenir a la position initial            
  delay(700);                        //attendre que le servo prenne sa position 
}
